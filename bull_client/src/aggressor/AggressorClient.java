...
public class AggressorClient extends JComponent implements ScriptableApplication, DisconnectListener, Callback {
	...
	protected String title = "Cobalt Strike[Bull🐂] By:l140w4n9 [精卫攻防实验室]";
	...
	public void setTeamServerAlias(String var1) {
		synchronized (this.data) {
			Map<String, String> var3 = (Map) this.data.get("options", Collections.emptyMap());
			var3.put("alias", var1);
		}

		String var2 = DataUtils.getTeamServerIP(this.data);
		Prefs.getPreferences().set("connection.profiles." + var2 + ".alias", var1);
	...
	public void disconnected() {
		...
		var2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent var1) {
				...
				AggressorClient.this.window.quit(AggressorClient.this, false);
			}
		});
		JButton var3 = new JButton("Reconnect");
		var3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent var1) {
				AggressorClient.this.stopTestConnection();
				Map var2 = AggressorClient.this.data.getMapSafe("options");
				String var3 = AggressorClient.this.window.getActiveAlias();
				AggressorClient.this.window.quit(AggressorClient.this, true);
				(new Connect(AggressorClient.this.window, true, var3)).dialogAction((ActionEvent) null, var2);
			}
		});
		JPanel var4 = new JPanel();
		var4.setLayout(new BorderLayout());
		var4.setBackground(Color.RED);
		JLabel var5 = new JLabel("Disconnected from server (" + getTeamServerAlias() + ")");
		var5.setFont(var5.getFont().deriveFont(1));
		var4.add(var5, "Center");
		JLabel var6 = new JLabel("");

		if (this.userInitiatedClose) {
			var7 = DialogUtils.center(var6, var3, var2);
		}
		else {
			var7 = DialogUtils.center(var6, var2);
		}

		var7.setBackground(Color.RED);
		var4.add(var7, "East");
		...
		if (!this.userInitiatedClose) {
			this.testConnection = new TestConnection(this.data.getMapSafe("options"), var6, var3);
			(new Thread((Runnable) this.testConnection)).start();
		}
	}
	...
	public void loadScripts() {
		try {
			...
			this.engine.loadScript("scripts/customizet.cna", CommonUtils.resource("scripts/customizet.cna"));
		}
		catch (Exception var5) {
			MudgeSanity.logException("Loading scripts/default.cna", var5, false);
		}

		Iterator<String> var1 = Prefs.getPreferences().getList("cortana.scripts").iterator();
	...
	public void setup(MultiFrame var1, TeamQueue var2, Map var3, Map var4) {
		...
		this.engine.register((Loadable) new TabBridge(this.engine, this.tabs));
		this.engine.register((Loadable) new GraphBridge(this.engine, this.tabs));
		this.engine.register((Loadable) new AggressorBridge(this, this.engine, this.tabs, var1, var2));
		this.engine.register((Loadable) new ToolBarBridge(this.tool));
		this.engine.register((Loadable) new TeamServerBridge(this.engine, var2));
		this.engine.register((Loadable) new DataBridge(this, this.engine, var2));
		this.engine.register((Loadable) new BeaconBridge(this, this.engine, var2));
		this.engine.register((Loadable) new BeaconTaskBridge(this));
		this.engine.register((Loadable) new ElevateBridge(this));
		this.engine.register((Loadable) new ElevatorBridge(this));
		this.engine.register((Loadable) new LateralBridge(this));
		this.engine.register((Loadable) new RemoteExecBridge(this));
		this.engine.register((Loadable) new UtilityBridge(this));
		this.engine.register((Loadable) new ReportingBridge(this));
		this.engine.register((Loadable) new EventLogBridge(this));
		this.engine.register((Loadable) new SafeDialogBridge(this));
		this.engine.register((Loadable) new PreferencesBridge(this));
		this.engine.register((Loadable) new ListenerBridge(this));
		this.engine.register((Loadable) new CovertVPNBridge(this));
		this.engine.register((Loadable) new ArtifactBridge(this));
		this.engine.register((Loadable) new DialogBridge(this));
		this.engine.register((Loadable) new SiteBridge(this));
		this.engine.register((Loadable) new AttackBridge());
...
/* Location:              C:\Users\liaowang\Desktop\CobaltStrikeModfiy\bullCs\bull_client\cs_bin\cat_client.jar!\aggressor\AggressorClient.class
