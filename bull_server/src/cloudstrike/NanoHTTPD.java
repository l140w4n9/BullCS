...
import static util.ModifiedBase64.modifiedBase64Decode;
import static util.SHA256Hash.bytesToHex;
import static util.SHA256Hash.sha256;
...
public class NanoHTTPD {
    ...
    public static final void print_error(String message) {
        System.out.println("\u001b[01;31m[-]\u001b[0m " + printableOnly(message));
    }

    public static final void print_info(String message) {
        System.out.println("\u001b[01;34m[*]\u001b[0m " + printableOnly(message));
    }

    public static final void print_warn(String message) {
        System.out.println("\u001b[01;33m[!]\u001b[0m " + printableOnly(message));
    }
    ...
    public void listen(boolean ssl, InputStream keystore, String password) throws IOException {
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                try {
                    NanoHTTPD.this.ss.setSoTimeout(60000);
    ...
    private class HTTPSession implements Runnable {
        ...
        public String checkGetUrl(String uri, String method, Properties header, Properties parms)
        {
            // 获取 timestamp 和 signature
            String timestamp = parms.getProperty("timestamp");
            String signatureEncode = parms.getProperty("signature");

            if (timestamp == null || signatureEncode == null) {
                return uri;
            }

            String signature = signatureEncode.replace('_', '+').replace('-', '/');

            byte[] signatureBytes;
            try {
                signatureBytes = modifiedBase64Decode(signature);
            } catch (IllegalArgumentException e) {
                return uri; // Base64 解析失败
            }

            String signatureStr = bytesToHex(signatureBytes);
            String sha256Timestamp = sha256(timestamp);
            if(!signatureStr.equalsIgnoreCase(sha256Timestamp))
                return uri;
        ...
        public String checkPostUrl(String uri, String method, Properties header, Properties parms)
        {
            // 获取 timestamp 和 signature
            String timestamp = parms.getProperty("timestamp");
            String signatureEncode = parms.getProperty("signature");
        ...
        public void run() {
            try {
                ...
                {
                    uri = checkGetUrl(uri, method, header, parms);
                }else
                {
                    uri = checkPostUrl(uri, method, header, parms);
//                    String newCookieHeard = header.getProperty("Cookie");
//                    int lastSlashIndex = newCookieHeard.lastIndexOf("-195eb4fb9b75a2");
//                    header.setProperty("Cookie", newCookieHeard.substring(0, lastSlashIndex));
                }

                parms.remove("timestamp");
        ...
        private void sendResponse(String status, String mime, Map header, InputStream data) {
            try {
                ...
                OutputStream outx = this.mySocket.getOutputStream();
                OutputStream out = new BufferedOutputStream(outx);
