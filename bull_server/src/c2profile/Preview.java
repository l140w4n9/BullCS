...
public class Preview implements Serializable {
    ...
    public String getServerSample(String var1) {
        ...
        try {
            ...
            byte[] var5 = CommonUtils.randomData(64);
            if (".http-post".equals(var1)) {
                var5 = new byte[0];
            }

            int var6 = this.c2profile.getInt(".data_jitter");
            if (var6 > 0 && !".http-stager".equals(var1)) {
                String var7 = var2 + "(.data_jitter:up.to." + var6 + ".bytes.of.random.data...)";
                var5 = CommonUtils.join(var5, var7.getBytes());
            }

            this.c2profile.apply(var1 + ".server", var4, var5);
            (new WebTransforms(this.c2profile)).filterResponse(var4);
            StringBuffer var13 = new StringBuffer();
            var13.append("HTTP/1.1 " + var4.status + "\n");
            Iterator var8 = var4.header.entrySet().iterator();
            ...
            if (var4.data != null) {
                var14 = new byte[var4.data.available()];
